...
for($x=1;$x<=12;$x++)
{
$sql = "SELECT t.tanggal_bulan,p.anggota_id, SUM(p.pencatatan_jumlah) as jumlah, SUM(p.pencatatan_jumlah*b.barang_harga) as Total_Harga
FROM pencatatan p, barang b, tanggal t
WHERE b.barang_id=p.barang_id
AND p.tanggal_id=t.tanggal_id
AND t.tanggal_bulan=".$x."
AND p.pencatatan_jumlah*b.barang_harga>=5000000";
...
for($x=1;$x<=12;$x++)
{
$sql = "SELECT a.anggota_nama, t.tanggal_bulan,p.anggota_id, p.pencatatan_jumlah*b.barang_harga as Total_Harga
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<tr>";
			...
			if($row["Total_Harga"]>=5000000)
			{
				echo "<td>Favorit</td>";	
			}
			else
				echo "<td>Tidak Favorit</td>";
		echo "</tr>";
